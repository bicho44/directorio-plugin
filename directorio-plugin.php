<?php
/*
Plugin Name: IMGD Directorio
Plugin URI: http://imgdigital.com.ar/portfolio/imgd_directorio/
...
define( 'DIRECTORIO_PLUGIN_PATH', plugin_dir_url( __FILE__ ) );

/**
 * Check if Piklist is activated and installed
 */
add_action('init', 'directorio_init_function');
function directorio_init_function()
{
    if(is_admin())
    {
 ...
 * Loading Translation
 */
function directorio_plugin_init() {
    $plugin_dir = basename(dirname(__FILE__)).'/languages';
    //echo '<h1>'.$plugin_dir.'</h1>';
    load_plugin_textdomain( 'imgd', false, $plugin_dir );
    wp_enqueue_script( 'directorio-directorio', DIRECTORIO_PLUGIN_PATH.'assets/js/jquery.fitvids.js', array( 'jquery' ), null, true );
}
add_action('plugins_loaded', 'directorio_plugin_init');
 ...
 */
add_filter('piklist_post_types', 'directorio_post_type');

function directorio_post_type($post_types)
{
    $singular = 'Item';
    $plural = 'Items';

    $post_types['imgd_directorio'] = array(
        'labels' => array(
                'name'               => _x( $singular, 'post type general name', 'imgd' ),
                'singular_name'      => _x( $singular, 'post type singular name', 'imgd' ),
                'menu_name'          => _x( 'Directorio', 'admin menu', 'imgd' ),
        ...
        ,'has_archive' => __('directorio','imgd')
        ,'menu_icon' => 'dashicons-tickets-alt'
        ,'page_icon' => 'dashicons-tickets-alt'
        ,'rewrite' => array(
            'slug' => __('directorio', 'imgd')
        )
...
function show_categoria($taxonomies)
{
  $labelUbica = array(
    'name'                          => __( 'Rubros', 'imgd' ),
    'singular_name'                 => __( 'Rubro', 'imgd' ),
    'search_items'                  => __( 'Buscar la Rubro', 'imgd' ),
    'popular_items'                 => __( 'Rubros Populares', 'imgd' ),
    'all_items'                     => __( 'Todas las Rubros', 'imgd' ),
    'parent_item'                   => __( 'Rubro pariente', 'imgd' ),
    'edit_item'                     => __( 'Editar Rubro', 'imgd' ),
    'update_item'                   => __( 'Actualizar Rubro', 'imgd' ),
    'add_new_item'                  => __( 'Agregar una nueva Rubro', 'imgd' ),
    'new_item_name'                 => __( 'Nueva Rubro', 'imgd' ),
    'separate_items_with_commas'    => __( 'Separe las Rubros con comas', 'imgd' ),
    'add_or_remove_items'           => __( 'Agregue o Borre Rubros', 'imgd' ),
    'choose_from_most_used'         => __( 'Elija alguna Rubro entre las más usadas', 'imgd' )
);
    $taxonomies[] = array(
        'post_type' => array('imgd_directorio', 'post', 'page')
        ,'name' => 'imgd_directorio_rubro'
    ...
    $taxonomies[] = array(
        'post_type' => array('imgd_directorio')
        ,'name' => 'imgd_directorio_ciudad'
        ,'show_admin_column' => true
        ,'configuration' => array(
            'hierarchical' => false
            ,'labels' => $labelciudad
...
function get_datos_video($post_ID){
  ...
  $nro = get_post_meta($post_ID , 'imgd_directorio_nro', true);
  $produccion = get_post_meta($post_ID , 'imgd_directorio_produccion', true);
  $director = get_post_meta($post_ID , 'imgd_directorio_director', true);
  $ano = get_post_meta($post_ID , 'imgd_directorio_ano', true);
